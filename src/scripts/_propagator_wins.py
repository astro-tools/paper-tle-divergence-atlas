...
import numpy as np
import pandas as pd
from _style import ALT_SHELL_ORDER, BUCKET_LABELS, BUCKET_SECONDS, bootstrap_by_sat
...
def compute(all_runs: pd.DataFrame) -> dict:
    """Top-level driver: pooled-per-Δt rows + per-cell rows."""
    pooled: list[dict] = []
    for bucket in BUCKET_SECONDS:
        cell = all_runs[all_runs["target_dt_sec"] == bucket]
    ...
    for shell in ALT_SHELL_ORDER:
        for bucket in BUCKET_SECONDS:
            cell = all_runs[
                (all_runs["alt_shell"] == shell) & (all_runs["target_dt_sec"] == bucket)
            ]
